...
- In shared config modules, let `src/_data/site.js` drive both feature toggles and feature-specific config. Prefer `enabled` for on/off behavior and `config` for payloads, for example `site.ga.enabled` + `site.ga.config` and `site.head.google.fonts.enabled` + `site.head.google.fonts.config`.
- Scope feature-specific config by the API being invoked instead of by ad hoc nested shapes. Examples: `site['setTemplateFormats']`, `site.metagen.config['addPlugin']['eleventy-plugin-metagen']`, `site.bundle.config['addPlugin']['EleventyRenderPlugin']`, `site.bundle.config['addBundle']['css']`, `site.css.config['addExtension']['css']`, `site.js.config['addExtension']['js']`, `site.js.config['build']['esbuild']`, `site.head.css.sanitize.config['addPassthroughCopy']['sanitize.css']`, `site.eleventyImg.config['Image']['@11ty/eleventy-img']`.
